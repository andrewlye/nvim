...
; <script type="..."> — replaces upstream's broken `set-lang-from-mimetype!`
; directive (nvim-treesitter master hasn't updated it for the nvim 0.11+
; match-shape API; calling it crashes the highlighter on any HTML buffer
; containing a typed <script> tag). See ~/.claude/.../memory/treesitter_query_shadows.md.
((script_element
  (start_tag
    (attribute
...
((script_element
  ...
  (raw_text) @injection.content)
  (#eq? @_attr "type")
  (#any-of? @_type "importmap" "application/json" "text/json")
...
((script_element
  ...
  (raw_text) @injection.content)
  (#eq? @_attr "type")
  (#any-of? @_type "application/typescript" "text/typescript")
